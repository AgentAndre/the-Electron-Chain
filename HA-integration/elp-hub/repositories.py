"""Repository layer — async data-access for the ELP-Hub.

One class per aggregate. All public methods are coroutines and return
plain dicts (or lists thereof) so they can be JSON-serialized directly
by FastAPI. No ORM — SQL is short enough that the indirection costs
...
class HeartbeatRepo:
    ...
    async def aggregate_minute(self, coop_id: str, minutes: int = 60) -> list[dict[str, Any]]:
        """Per-minute aggregation of total surplus and PV across the coop."""
    ...
    async def prune(self, older_than_seconds: int = 86400) -> int:
        """Drop heartbeats older than 1d (raw resolution) — keep aggregations."""
        async with db() as conn:
            cur = await conn.execute(
