"""SQLite schema for The Electron Chain ELP-Hub addon.

Single-file DB at $ELP_DB_PATH (default /data/elp.sqlite). The /data volume
is persisted by HA Supervisor, so the schema survives addon restarts and
HA upgrades.

Schema versioning lives in `schema_version`; migrations stay additive so
older addon installs roll forward without manual intervention.
"""
...
# v3 — encrypted secrets vault. Stores the addon-side wallet seed (and any
# other secret) wrapped by Fernet with a key derived from a user passphrase.
# Only ciphertext touches disk; the passphrase is never persisted.
DDL_V3 = """
...
async def init_schema() -> None:
    ...
    async with db() as conn:
        ...
        if current < 3:
            await conn.executescript(DDL_V3)
